...
def set_xls_name(stat_type):
    # xls doc set
    book_name_xls = 'Data with ' + stat_type + '.xlsx'
...
def get_selected_data(y, z):
    for x in year_list:
        url = 'https://basketball.realgm.com/international/league/5/Australian-NBL/stats/{}/{}/All/All/points/{}/desc/1?pace_adjustment='.format(
            x, y, z)
...
def delete_rows():
    filename = 'Data with All year Player_data.xlsx'
    df = pd.read_excel(filename)  # read excel all contents
    df1 = df.drop(['Pos'], axis=1)  # delete pos column
    df1 = df1.drop(['Team'], axis=1)  # delete pos column
    df1.to_excel('Data with All year Player_data.xlsx', sheet_name='Player_data', index=False)
